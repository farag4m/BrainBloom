...
public final class NotificationService {
    ...
    public func scheduleWarningNotification(for rule: AppRule) {
        ...
        let request = UNNotificationRequest(
            identifier: "intentblock.warning.\(rule.id)",
            content: content,
            trigger: nil
        )
    ...
    public func scheduleLockedNotification(for rule: AppRule) {
        ...
        content.body = "You've used your allowance for \"\(rule.name)\". Open IntentBlock to continue."
        content.sound = .default
        content.userInfo = ["ruleID": rule.id.uuidString]

        let request = UNNotificationRequest(
            identifier: "intentblock.locked.\(rule.id)",
            content: content,
            trigger: nil
        )
        UNUserNotificationCenter.current().add(request)
    ...
    public func scheduleGraceEndedNotification(for rule: AppRule) {
        ...
        let request = UNNotificationRequest(
            identifier: "intentblock.grace_ended.\(rule.id).\(UUID())",
            content: content,
            trigger: nil
        )
        UNUserNotificationCenter.current().add(request)
    ...
    public func cancelAllNotifications(for ruleID: UUID) {
        let identifiers = [
            "intentblock.warning.\(ruleID)",
            "intentblock.locked.\(ruleID)"
        ]
