    ...
    private var statusRow: some View {
        ...
        case .locked:
            HStack(spacing: 8) {
                ...
                Text("Limit reached. Take a short break.")
...
#Preview("Active Rule") {
    let rule = AppRule(
        ...
        policy: .daily(minutes: 60)
    )
    RuleCardView(
        item: RuleDisplayItem(rule: rule, badge: .active, activeSession: nil),
        onToggle: {}, onUpdate: { _ in }, onDelete: {}
    )
    .padding()
}
...
#Preview("Locked Rule") {
    let rule = AppRule(
        name: "TikTok",
        selection: FamilyActivitySelection(),
        policy: .recurring(minutes: 10, everyHours: 1)
    )
    RuleCardView(
        item: RuleDisplayItem(rule: rule, badge: .locked, activeSession: nil),
        onToggle: {}, onUpdate: { _ in }, onDelete: {}
    )
    .padding()
}

#Preview("Disabled Rule") {
    let rule = AppRule(
        name: "News",
        selection: FamilyActivitySelection(),
    ...
    RuleCardView(
        item: RuleDisplayItem(rule: rule, badge: .disabled, activeSession: nil),
        onToggle: {}, onUpdate: { _ in }, onDelete: {}
    )
    .padding()
}
