...
class IntentBlockMonitor: DeviceActivityMonitor {
    ...
    private func applyShield(for ruleID: UUID) {
        ...
        let settingsStore = ManagedSettingsStore(
            named: ManagedSettingsStore.Name("intentblock-\(ruleID.uuidString)")
        )
    ...
    private func postNotification(type: NotifType, ruleID: UUID) {
        ...
        case .locked:
            ...
            content.body = isRecurring
                ...
                : "You've reached your daily limit. Open IntentBlock to continue."
            content.userInfo = ["ruleID": ruleID.uuidString]
        }

        UNUserNotificationCenter.current().add(
            UNNotificationRequest(
                identifier: "intentblock-\(type)-\(ruleID)",
                content: content,
                trigger: nil
            )
        )
